...
\entry{AVL Tree}
{Can be augmented to support in $O(\log_2 n)$ time: range queries/updates (similar to a segment tree), insert at position $n$/query for position $n$, order statistics, etc.}
...
{snippets/datastructures/suffixarray.cpp}{$O(n \log_2 n)$}{}
...
\entry{Bellmann-Ford extended algorithm}
...
{snippets/graphs/centroid.cpp}{$O(n \log_2 n)$}{}
...
\entry{Edmonds Karp using Dijkstra Ragnar}{}
...
% \& crashes pdf-viewer index generator. Something due to hyperref
