...
\begin{document}
  ...
  \section{Helpers}
	...
	\entry{QuickSelect}{Running time is expected, quadratic in the worst case. Alternatingly breaks ties left and right, so it should be pretty resilient to edge cases. Note that the vector is changed in the process. Recursion depth is $O(n \log_2 n)$.}{snippets/helpers/quickselect.cpp}{$O(n)$}{}
