...
\begin{document}
  ...
  \section{Graph algorithms}
    ...
    \begin{description}
        ...
        \item[Kirchhoff's theorem]: Define a $V \times V$ matrix $M$ as: $M_{ij} = deg(i)$ if $i == j$, $M_{ij} = - 1$ if $\{i, j\} \in E$, $M_{ij} = 0$ otherwise. Then the number of distinct spanning trees equals $\det(M_{ii})$ for any $i$.
    \end{description}
