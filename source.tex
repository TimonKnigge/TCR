...
\subsection{Johnson's reweighting algorithm}
...
\entry{Convex Hull optimization}{
	When $a[j+1] < a[j]$ and $x[i+1] > x[i]$ (otherwise sort
	$x$):
\begin{align*}
	dp[k][i] &= \min_{j<i}\left\{a[j]\cdot x[i] + dp[k-1][j]\right\} + c[i]\\
	dp[i] &= \min_{j<i}\left\{a[j]\cdot x[i] + dp[j]\right\} + c[i]
\end{align*}
