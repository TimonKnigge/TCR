...
\entry{Dominating Set}{}{./snippets/datastructures/dominating-set.cpp}
...
for $k=3000$ and $n=10^{18}$.}{snippets/math/lin-rec.cpp}

\entry{Matrix Solver}{}{snippets/math/matrix_solve.cpp}

\entry{Matrix Exponentiation}{}{snippets/math/matrix_exp.cpp}
...
to approximate the minimum $k$-cut problem: just remove the $k$ lightest edges
from the tree. This is guaranteed to give a $2-2/k$ approximation.
...
\begin{itemize}
	...
	\item If a cricle passing through two points $p \in S$ does not contain
		any other points in $S$ in its interior, the segment connecting these
		two points is part of the Delaunay triangulation.
	\item The \textit{nearest neighbour graph} is a subset of the Delaunay
		triangulation.
	\item In $d=2$, $D$ is a geometric spanner, the shortest path through $D$
		between any two points is at no more than $\frac{4\pi}{3\sqrt{3}}$
		times the Euclidian distance.
